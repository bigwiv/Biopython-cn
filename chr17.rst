...
than one track 每 for example show the genes on one, and repeat regions
...
-  ``NC_002703`` 每 Lactococcus phage Tuc2009, complete genome (38347 bp)
-  ``AF323668`` 每 Bacteriophage bIL285, complete genome (35538 bp)
-  ``NC_003212`` 每 *Listeria innocua* Clip11262, complete genome, of
...
Now to draw them 每 this time we add three tracks to the diagram, and
...
links between similar proteins 每 which is what we will do in the next
section.
...
``gd_diagram.draw(...)`` line 每 see the finished example script
...
instead 每 one way to do that is to allocate space for empty tracks.
...
You can control the tick marks to show the scale 每 after all every graph
