...
in which x, ? are the sample mean of *x* and *y* respectively, and
考\ :sub:`*x*`, 考\ :sub:`*y*` are the sample standard deviation of *x*
...
coefficient, except that the sample means x, ? are set equal to zero.
...
Kendall＊s 而 is another example of a non-parametric similarity measure.
It is similar to the Spearman rank correlation, but instead of the ranks
themselves only the relative ranks are used to calculate 而 (see Snedecor
& Cochran [`29 <#snedecor1989>`__\ ]).

We can define a distance measure corresponding to Kendall＊s 而 as

+--------------------------+
| *d*\ :sub:`K`?√?1???而.   |
+--------------------------+

As Kendall＊s 而 is always between -1 and 1, the corresponding distance
...
correlation and Kendall＊s 而, weights do not have a well-defined meaning
...
The parameter 而 is a parameter that decreases at each iteration step. We
...
而\ :sub:`init` is the initial value of 而 as specified by the user, *i*
...
-  ``inittau`` (default: ``0.02``)
    The initial value for the parameter 而 that is used in the SOM
