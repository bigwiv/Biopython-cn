...
calcium atoms are called ``¡¯CA..¡¯`` in order to distinguish them from C¦Á
...
(e.g. an amino acid C ¦Á atom is labeled ¡°.CA.¡± in a PDB file, where the
...
suppose that you would like to find the position of a Gly residue¡¯s C¦Â
atom, if it had one. Rotating the N atom of the Gly residue along the
C¦Á-C bond over -120 degrees roughly puts it in the position of a virtual
C¦Â atom. Here¡¯s how to do it, making use of the ``rotaxis`` method
...
C¦Á atoms, you do not care that some residues have a disordered side
...
object or to get a list of C¦Á atoms as well. Polypeptides can be built
using a C-N or a C¦Á-C¦Á distance criterion.
...
[`20 <#hamelryck2005>`__\ ]. Basically, it counts the number of C¦Á atoms
...
HSE comes in two flavors: HSE¦Á and HSE¦Â. The former only uses the C¦Á
atom positions, while the latter uses the C¦Á and C¦Â atom positions. The
...
dictionaries that map a ``Residue`` object to its corresponding HSE¦Á,
HSE¦Â and contact number values.
...
for it ¡ª free for academic use, see
...
which maps ``Residue`` objects to corresponding (residue depth, C¦Á
depth) tuples. The C¦Á depth is the distance of a residue¡¯s C¦Á atom to
