...
-  The ability to parse bioinformatics files into Python utilizable data
   ...
   -  Blast output 每 both from standalone and WWW Blast
   -  Clustalw
   -  FASTA
   -  GenBank
   -  PubMed and Medline
...
-  Code to deal with popular on-line bioinformatics destinations such
   ...
   -  NCBI 每 Blast, Entrez and PubMed services
   -  ExPASy 每 Swiss-Prot and Prosite entries, as well as Prosite
      searches
...
每 but its better to make sure everything seems to be working.)
...
#. *Why has my script using* ``Bio.Entrez.efetch()`` *stopped working?*
   ...
   are now stricter about how to provide a list of IDs 每 Biopython 1.59
...
#. *Why doesn＊t* ``Bio.Blast.NCBIWWW.qblast()`` *give the same results
   as the NCBI BLAST website?*
    You need to specify the same options 每 the NCBI often adjust the
