...
Call the ``write`` function with a file name or handle ¡ª here we use
standard output, to see what would be written ¡ª and the format
...
-  A **TreeElement instance**, which tree elements will match by
   identity ¡ª so searching with a Clade instance as the target will find
   that clade in the tree;
-  A **string**, which matches tree elements¡¯ string representation ¡ª in
...
-  A **dictionary** where keys are tree element attributes and values
   ...
   match each of the given attribute values ¡ª think ¡°and¡±, not ¡°or¡±.
 ...
 **terminal**
    ¡ª A boolean value to select for or against terminal clades (a.k.a.
...
**order**
    ¡ª Tree traversal order: ``"preorder"`` (default) is depth-first
 ...
 **``is_bifurcating``**
    ...
    tree.
**``is_monophyletic``**
    Test if all of the given targets comprise a complete subclade ¡ª
...
**``is_parent_of``**
    True if target is a descendent of this tree ¡ª not required to be a
...
**``split``**
    ...
    from 0) ¡ª for example, splitting a clade named ¡°A¡± produces the
...
**Bio.Nexus port**
    ...
    been ported to Bio.Phylo classes ¡ª notably, calculating a consensus
